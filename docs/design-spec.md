...
| 模块 | 类名 | 职责 | 文件位置 |
|------|------|------|----------|
| 配置管理 | ConfigManager | 配置文件读写、默认配置合并、嵌套键值访问 | app/core/config.py |
| 日志系统 | Logger | 日志记录（文件+控制台双通道）、按天切割、console=False兼容 | app/core/logger.py |
| 权限检测 | AdminChecker | 管理员权限检测、UAC提权、require_admin装饰器 | app/core/permission.py |
| 线程池 | ThreadPool | 异步任务执行、并发控制、TaskThread封装 | app/core/thread_pool.py |

#### 3.3.2 表示层 (app/ui)

按功能分为 9 个导航分组，共 30+ 页面组件：

| 分组 | 页面 | 说明 | 文件 | 主要类名 |
|------|------|------|------|----------|
| 概览 | 仪表盘 | 系统资源监控、网络状态总览 | dashboard.py | DashboardPage, StatCard, MiniChart, ConnectionCheckWidget |
| 诊断检测 | 网络健康 | 一键网络健康体检 | network_health.py | NetworkHealthPage, HealthCheckWorker |
| 诊断检测 | Ping测试 | 单个/批量/网段/TCP Ping | ping_test.py | PingTestPage, PingWorker |
| 诊断检测 | 路由追踪 | Traceroute 路由追踪 | traceroute.py | TraceroutePage, TracerouteWorker |
| 诊断检测 | 端口扫描 | TCP端口扫描、服务识别 | port_scan.py | PortScanPage, PortScanWorker |
| 诊断检测 | 主机发现 | 局域网主机扫描、拓扑展示 | host_discovery.py | HostDiscoveryPage, HostDiscoveryWorker, TopologyNode, TopologyScene |
| 诊断检测 | 摄像头扫描 | RTSP/ONVIF摄像头检测 | camera_scan.py | CameraScanPage, CameraScanWorker |
| 速度测试 | 外网测速 | 基于HTTP下载的外网测速 | speed_external.py | SpeedExternalPage |
| 速度测试 | 内网测速 | 双引擎内网测速（内置/iperf3） | speed_internal.py | SpeedInternalPage, InternalSpeedWorker |
| 速度测试 | 会话测试 | 并发连接压力测试 | speed_session.py | SpeedSessionPage, SessionTestWorker |
| 网络分析 | 数据包抓包 | scapy数据包捕获与分析 | packet_capture.py | PacketCapturePage, CaptureWorker |
| 网络分析 | 协议分析 | 网络协议深度分析 | protocol_analysis.py | ProtocolAnalysisPage, ProtocolWorker |
| 网络分析 | DHCP检测 | DHCP服务检测与网卡信息 | dhcp_check.py | DHCPCheckPage, DHCPCheckerWorker |
| 实用工具 | 子网计算 | IPv4/IPv6子网掩码计算 | subnet_calc.py | SubnetCalculatorPage |
| 实用工具 | IP信息检测 | IP地理位置查询、冲突检测 | ip_info.py | IPInfoCheckPage, PublicIPWorker, IPConflictScanner |
| 实用工具 | MAC地址工具 | MAC厂商查询、ARP缓存表 | mac_tool.py | MACAddressToolPage |
| 实用工具 | 路由表 | 系统路由表查看与管理 | route_table.py | RouteTablePage |
| 实用工具 | 连接测试 | DNS/Telnet/MTU/SSL等11项测试 | connection_test.py | ConnectionTestPage, DNSQueryThread |
| 网络服务 | 网络服务 | FTP客户端/服务端、HTTP/TFTP服务 | network_service.py | NetworkServicePage, FTPThread, FTPHandler, HTTPFileServer, TFTPHandler |
| 网络服务 | 本机设置 | 系统工具、网络配置快捷入口 | local_settings.py | LocalSettingsPage |
| 网络服务 | 防火墙配置 | Windows防火墙规则管理 | firewall.py | FirewallPage |
| 监控运维 | 链路监控 | 网络链路状态监控与告警 | link_monitor.py | LinkMonitorPage, LinkMonitorWorker |
| 监控运维 | 流量监控 | 实时流量统计与可视化 | traffic_monitor.py | TrafficMonitorPage, TrafficChart |
| 远程工具 | 远程终端 | SSH/Telnet多会话终端 | remote_terminal.py | RemoteTerminalPage |
| 远程工具 | 远程桌面 | RDP远程桌面连接管理 | remote_desktop.py | RemoteDesktopPage |
| 远程工具 | 串口调试 | RS232/USB串口通信调试 | serial_debug.py | SerialDebugPage |
| 智能系统 | 关于和支持 | 软件信息、版本说明 | about.py | AboutPage |
| - | 占位页面 | 未实现功能的占位提示 | placeholder.py | PlaceholderPage |
...
#### 3.6.5 subprocess 安全调用模式（V1.2 新增）
...
#### 3.8.3 console=False 模式兼容（V1.2 关键修复）
...
### 4.1 配置管理模块 (app/core/config.py)

**类名**: ConfigManager

**职责**: 统一管理应用配置，支持JSON持久化存储和嵌套键值访问。
...
| 方法 | 说明 |
|------|------|
| load() | 从~/.network-toolkit/config.json加载配置 |
| save() | 保存配置到JSON文件 |
| get(key, default) | 支持嵌套键值访问，如get("ping.default_count") |
| set(key, value) | 设置配置项并自动保存 |

### 4.2 日志系统模块 (app/core/logger.py)

**类名**: Logger

**职责**: 统一日志管理，支持文件和控制台输出，兼容console=False模式。
...
### 4.3 权限检测模块 (app/core/permission.py)

**类名**: AdminChecker

**职责**: Windows管理员权限检测与提权。
...
### 4.4 线程池模块 (app/core/thread_pool.py)

**类名**: ThreadPool, TaskThread, Worker
...
### 4.5 DHCP检测模块（V1.2 重构）

**文件**: app/ui/dhcp_check.py
**类名**: DHCPCheckPage, DHCPCheckerWorker
...
**文件**: app/ui/speed_internal.py
**类名**: SpeedInternalPage, InternalSpeedWorker
...
#### 4.6.3 服务端引擎选择（V1.2 新增）

服务端面板添加了引擎选择单选按钮：
- **内置引擎**：启动 Python socket 服务端
- **iperf3**：调用 `iperf3 -s -p <port>` 启动标准服务端，使用 `CREATE_NO_WINDOW` 隐藏窗口

### 4.7 FTP客户端模块（V1.2 完整重写）

**文件**: app/ui/network_service.py
**类名**: FTPThread, NetworkServicePage
...
**文件**: app/ui/ping_test.py
**类名**: PingTestPage, PingWorker
...
**文件**: app/ui/port_scan.py
**类名**: PortScanPage, PortScanWorker

**功能**：TCP端口扫描，支持多线程并发扫描。

### 4.10 路由追踪模块

**文件**: app/ui/traceroute.py
**类名**: TraceroutePage, TracerouteWorker

**功能**：逐跳显示路由路径，支持多线程并发追踪。

### 4.11 主机发现模块

**文件**: app/ui/host_discovery.py
**类名**: HostDiscoveryPage, HostDiscoveryWorker, TopologyNode, TopologyScene

**功能**：局域网主机扫描，支持拓扑图展示。

### 4.12 摄像头扫描模块

**文件**: app/ui/camera_scan.py
**类名**: CameraScanPage, CameraScanWorker

**功能**：RTSP/ONVIF摄像头检测。

### 4.13 网络健康模块

**文件**: app/ui/network_health.py
**类名**: NetworkHealthPage, HealthCheckWorker

**功能**：一键网络健康体检。

### 4.14 外网测速模块

**文件**: app/ui/speed_external.py
**类名**: SpeedExternalPage

**功能**：基于HTTP下载的外网测速。

### 4.15 会话测试模块

**文件**: app/ui/speed_session.py
**类名**: SpeedSessionPage, SessionTestWorker

**功能**：并发连接压力测试。

### 4.16 数据包抓包模块

**文件**: app/ui/packet_capture.py
**类名**: PacketCapturePage, CaptureWorker

**功能**：scapy数据包捕获与分析。

### 4.17 协议分析模块

**文件**: app/ui/protocol_analysis.py
**类名**: ProtocolAnalysisPage, ProtocolWorker

**功能**：网络协议深度分析。

### 4.18 子网计算模块

**文件**: app/ui/subnet_calc.py
**类名**: SubnetCalculatorPage

**功能**：IPv4/IPv6子网掩码计算。

### 4.19 IP信息检测模块

**文件**: app/ui/ip_info.py
**类名**: IPInfoCheckPage, PublicIPWorker, IPConflictScanner

**功能**：IP地理位置查询、冲突检测。

### 4.20 MAC地址工具模块

**文件**: app/ui/mac_tool.py
**类名**: MACAddressToolPage

**功能**：MAC厂商查询、ARP缓存表。

### 4.21 路由表模块

**文件**: app/ui/route_table.py
**类名**: RouteTablePage
...
**文件**: app/ui/connection_test.py
**类名**: ConnectionTestPage, DNSQueryThread

**功能**：DNS/Telnet/MTU/SSL等11项测试。

### 4.23 防火墙配置模块

**文件**: app/ui/firewall.py
**类名**: FirewallPage
...
**文件**: app/ui/local_settings.py
**类名**: LocalSettingsPage

**功能**：系统工具、网络配置快捷入口。

### 4.25 链路监控模块

**文件**: app/ui/link_monitor.py
**类名**: LinkMonitorPage, LinkMonitorWorker

**功能**：网络链路状态监控与告警。

### 4.26 流量监控模块

**文件**: app/ui/traffic_monitor.py
**类名**: TrafficMonitorPage, TrafficChart

**功能**：实时流量统计与可视化。

### 4.27 远程终端模块

**文件**: app/ui/remote_terminal.py
**类名**: RemoteTerminalPage

**功能**：SSH/Telnet多会话终端。

### 4.28 远程桌面模块

**文件**: app/ui/remote_desktop.py
**类名**: RemoteDesktopPage

**功能**：RDP远程桌面连接管理。

### 4.29 串口调试模块

**文件**: app/ui/serial_debug.py
**类名**: SerialDebugPage
